...
/* Structure Definations for LIFO Stack */

# define stack_nil (treiberstack_node*) 0

struct treiberstack_node {	
   _Atomic struct treiberstack_node *next;
   _Atomic _Bool blocked;
};

struct lifolock{
    _Atomic struct treiberstack_node *top;
};

struct treiberstack_node * myNode;
struct lifolock * mcs_stack_lock;
...
void* lock_LIFO_counter(void* arg);
//void lifo_lock_function(lifolock_t *l, lifo_node_t *me); 
//void lifo_unlock_function(lifolock_t *l, lifo_node_t *me);
void mcs_stack_lock_function(struct treiberstack_node* myNode,struct lifolock * lock);
void mcs_stack_unlock_function(struct treiberstack_node* myNode,struct lifolock *lock);
...
int main(int argc, char *argv[])
{
    ...
    if (((strcmp(lock_selected,"mcs"))) == 0)
	{
        ...
        for(long i=0; i< THREAD_CNT;i++)
        {
            int pid = pthread_create(&threads[i],NULL,lock_mcs_counter,(void *)i);
            if(pid){ printf("Error creating thread\n");
                    exit(-1);
            }
        }
	...
	/* Run using LIFO lock  */
    if (((strcmp(lock_selected,"LIFO"))) == 0)
	{
        /* thread creation */
        for(long i=0; i< THREAD_CNT;i++)
        {
            int pid = pthread_create(&threads[i],NULL,lock_LIFO_counter,(void *)i);
            if(pid){ printf("Error creating thread\n");
                    exit(-1);
            }
        }
...
void* lock_LIFO_counter(void* arg)
{
	...
	for(int i = 0; i<ITERATIONS; i++)
	{
		mcs_stack_lock_function(&mcs_stack_lock,&myNode);
		lock_cntr++;
		mcs_stack_unlock_function(&mcs_stack_lock,&myNode);
	}
	if(tid==1){clock_gettime(CLOCK_MONOTONIC,&end);}
}
...
void mcs_stack_lock_function(struct treiberstack_node* myNode,struct lifolock * lock){
    //myNode->next = NULL;//2->next = NULL
	atomic_store_explicit(&myNode->next,NULL,memory_order_relaxed);
    //myNode->wait = true;//2->wait = true
	atomic_store_explicit(&myNode->blocked,true,memory_order_relaxed);
    
    // struct treiberstack_node *oldTop = top.load(ACQUIRE);
	struct treiberstack_node *oldTop = atomic_exchange_explicit(&lock->top, myNode, memory_order_acq_rel);
	
	/* Wait for predecessor Stack node */
    if(oldTop!=NULL){
		/* Push My Node into Stack */
		atomic_init(&myNode->blocked,true);
		atomic_store_explicit(&oldTop->next,myNode,memory_order_acq_rel);
		/* Push into LIFO stack Complete */
        while(myNode->blocked == true){}
    }
}

void mcs_stack_unlock_function(struct treiberstack_node* myNode,struct lifolock *lock){

	struct treiberstack_node* currentTop = atomic_load_explicit(&lock->top,memory_order_acq_rel);
    if(currentTop==NULL){
		return;
	}
	
	atomic_store_explicit(&currentTop->blocked,false,memory_order_acq_rel);
	/* Pop node from stack*/
...
//  void lifo_lock_function(lifolock_t *l, lifo_node_t *me)
//  {
//    /* initialize my stack node */
//    atomic_init(&me->next, stack_nil);
//    /* Installing my stack node at queue head . Check predessesro and check if any predessesor is still waiting */
//    lifo_node_t *predecessor = atomic_exchange_explicit(&l->head, me, memory_order_acq_rel);
//    /* Wait for predecessor Stack node */
//    if (predecessor != stack_nil) {
...
// void lifo_unlock_function(lifolock_t *l, lifo_node_t *me)
//  {
//    lifo_node_t *successor = atomic_load_explicit(&me->next, memory_order_acquire);
 
//    if (successor == stack_nil) {
//      lifo_node_t *oldme = me;
//      if (atomic_compare_exchange_strong_explicit(&l->head, &oldme, stack_nil,
//                                                  memory_order_release,
//                                                  memory_order_relaxed)) {
//        return;
//      }
//      while (stack_nil == (successor = atomic_load_explicit(&me->next, memory_order_acquire)));
//    }
